...
public class OSCPacketDispatcher {
	...
	/**
	 * @return the selectorToListener
	 */
	private synchronized Map<AddressSelector, OSCListener> getSelectorToListener() {
		return selectorToListener;
	}
	 ...
	 */
	public synchronized void addListener(AddressSelector addressSelector, OSCListener listener) {
		getSelectorToListener().put(addressSelector, listener);
	}
	
	
	public synchronized void removeListener(AddressSelector addressSelector) {
		getSelectorToListener().remove(addressSelector);
	}
	...
	private synchronized void dispatchBundle(OSCBundle bundle) {
	...
	private synchronized void dispatchMessage(OSCMessage message, Date time) {
		for (final Entry<AddressSelector, OSCListener> addrList : getSelectorToListener().entrySet()) {
