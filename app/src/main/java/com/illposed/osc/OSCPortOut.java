...
//	private InputStream tcpInputStream;
	 ...
	 */
	public OSCPortOut(InetAddress address, int port, DatagramSocket socket) throws SocketException {
		super(socket, port);
	...
	/**
	 * Create an OSCPort for TCP that sends to address:port using a specified socket.
	 * @param address the TCP address to send to
	 * @param port the TCP port to send to
	 * @param socket the Socket to send from
	 * @throws SocketException 
	 */
	public OSCPortOut(InetAddress address, int port, Socket socket) throws SocketException {
		super(socket, port);
		this.address = address;
	}
...
//	}
	 ...
	 * @throws IOException if a (UDP) socket I/O error occurs
	 */
	public int send(OSCPacket aPacket) throws IOException {
		final byte[] byteArray = aPacket.getByteArray();
		...
		if (isUDPPort()) {
			...
			// TCP is not working!
			PrintWriter out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(getTCPSocket().getOutputStream())), true);
...
//	public void sendTCP(OSCPacket aPacket) throws IOException {
//		final byte[] byteArray = aPacket.getByteArray();
//		final DatagramPacket packet =
//				new DatagramPacket(byteArray, byteArray.length, address, getPort());
////		getTCPSocket().send(packet);
//		// TODO connect and init the input and output stream.
//	}
}
