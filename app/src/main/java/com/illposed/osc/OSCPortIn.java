...
public class OSCPortIn extends OSCPort implements Runnable {
	...
	/**
	 * Create an OSCPort for TCP that listens using a specified socket.
	 * @param socket Socket to listen on.
	 * @throws SocketException 
	 */
	public OSCPortIn(Socket socket) throws SocketException {
		super(socket, socket.getLocalPort());
	...
	public void run() {
		...
		if (isUDPPort()) {
			...
			final DatagramSocket socket = getUDPSocket();
			while (listening) {
				try {
					try {
					...
					// as this could easily lead to a never ending 
					// series of exceptions thrown 
					// (due to the non-exited while loop), 
					// and because the user of the lib may want 
					// to handle this case himself
				}
			}
		} else {
//			final DatagramPacket packet = new DatagramPacket(buffer, BUFFER_SIZE);
			int numberOfBytesRead = -1;
			final Socket socket = getTCPSocket();
...
//						String message;
						try {
							numberOfBytesRead = objectInputStream.read(buffer, 0, BUFFER_SIZE);
						} catch (SocketException ex) {
							if (listening) {
								throw ex;
							} else {
	...
	public void removeListener(String addressSelector) {
		this.removeListener(new OSCPatternAddressSelector(addressSelector));
	}
	
	public void removeListener(AddressSelector addressSelector) {
		dispatcher.removeListener(addressSelector);
	}
